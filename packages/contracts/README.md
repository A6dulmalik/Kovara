...
| StorageKey Variant      | Key Type         | Value Type         | Purpose                                                           |
| ----------------------- | ---------------- | ------------------ | ----------------------------------------------------------------- |
| `Profile(Address)`      | User address     | `Profile`          | Profile data by user address                                      |
| `UsernameIndex(String)` | Username string  | `Address`          | Reverse index from username to owner address (ensures uniqueness) |
| `Following(Address)`    | Follower address | `Vec<Address>`     | List of accounts a user is following                              |
| `Followers(Address)`    | Followee address | `Vec<Address>`     | List of accounts following a user                                 |
| `Blocks(Address)`       | Blocker address  | `Map<Address, ()>` | Set of users blocked by a user                                    |
| `Post(u64)`             | Post ID          | `Post`             | Post body and metadata by unique post ID                          |
| `AuthorPosts(Address)`  | Author address   | `Vec<u64>`         | List of post IDs created by an author                             |
...
Short-lived state with automatic TTL expiration:

| StorageKey Variant          | Key Type          | Value Type | Purpose                                                              |
| --------------------------- | ----------------- | ---------- | -------------------------------------------------------------------- |
| `TipCooldown(u64, Address)` | (post_id, tipper) | `u32`      | Last ledger sequence number a user tipped a post (enforces cooldown) |

### Data Structures

All complex types are defined as Soroban contract types with full serialization support:
...
All persistent entries have their TTL extended (bumped) to ~30 days (535,000 ledgers) on every read or write to keep active data alive on-chain. Instance storage is bumped on every mutating operation.

## Reproducible Testnet Deployment Verification

From `packages/contracts/contracts/linkora-contracts`, install and authenticate the Stellar CLI, then run:

```sh
STELLAR_SOURCE_ACCOUNT=alice make deploy-verify
```

The command builds the WASM, deploys it to `STELLAR_NETWORK` (defaulting to `testnet`), fetches the deployed artifact, compares SHA-256 hashes, and writes the verified `CONTRACT_ID` and `WASM_SHA256` to `deployment-testnet.env`. Set `DEPLOYMENT_OUTPUT` to choose another record file. The generated file contains deployment identifiers and must not be committed.
